    ...
    for (ClusterStats clusterStats : clustersStats) {
      if (!clusterStats.isHealthy()) {
        Integer currentFailureCount =
            clusterHealthFailureCount.getOrDefault(clusterStats.getClusterId(), 0);
        ...
        if (clusterStats.getQueuedQueryCount() > MAX_THRESHOLD_QUEUED_QUERY_COUNT) {
          Integer currentFailureCount =
              clusterMaxQueryCountFailureCount.getOrDefault(clusterStats.getClusterId(), 0);
          currentFailureCount += 1;
          if (currentFailureCount >= DEFAULT_CONSECUTIVE_FAILURE_ALERT_COUNT) {
        ...
        if (clusterStats.getNumWorkerNodes() < 1) {
          Integer currentFailureCount =
              clusterNoWorkerFailureCount.getOrDefault(clusterStats.getClusterId(), 0);
          currentFailureCount += 1;
          if (currentFailureCount >= DEFAULT_CONSECUTIVE_FAILURE_ALERT_COUNT) {
